...
public:
	...
	void TeleopPeriodic() {
		...
		_sb += "\tout:";
		_sb += std::to_string(motorOutput);
		_sb += "\tpos:";
		_sb += std::to_string(_talon.GetSelectedSensorPosition(kPIDLoopIdx));
		...
		if (_talon.GetControlMode() == ControlMode::Position) {
			...
			_sb += "\terrNative:";
			_sb += std::to_string(_talon.GetClosedLoopError(kPIDLoopIdx)));
			_sb += "\ttrg:";
			_sb += std::to_string(targetPositionRotations);
		}
